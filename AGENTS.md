...
| the version a release pull request carries — the labels merged since the last tag, raising what semver-checks found | `.github/next-version.test.sh`, a step of the `check` job: `.github/next-version.sh` over a table of labels and proposals. Publishing, the tag and the GitHub release are release-plz's, and are seen working on the first real release |
